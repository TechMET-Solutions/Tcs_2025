...
export default function ProductRegistration() {
  ...
  const saveProduct = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        alert(
          `✅ Product ${modalMode === "edit" ? "Updated" : "Saved"} Successfully`,
        );
  ...
  const handleDownloadPDF = async () => {
    const element = batchRef.current;
    ...
    <div className="p-6">
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-6 flex flex-wrap items-center gap-4">
        ...
        <div className="w-[180px] relative">
          ...
          >
            ...
            {[...new Set(productList.map((p) => p.size))]
              .filter(Boolean)
              .map((size, i) => (
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 px-2">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Manage and track your inventory stock and logistics.
          </p>
        </div>
        {(role === "admin" ||
          role === "superadmin" ||
          permissions?.["Product Registration_Add"] === true) && (
          <button
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredProducts.length === 0 ? (
                <tr>
                  <td
                    colSpan="7"
                    className="text-center py-24 text-slate-400 font-medium italic text-lg"
                  >
                ...
                filteredProducts.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4">
                          <div className="h-14 w-14 rounded-2xl bg-slate-100 overflow-hidden border-2 border-white shadow-sm flex-shrink-0 group-hover:scale-105 transition-transform">
                            {item.image_url ? (
                              ...
                              <div className="h-full w-full flex items-center justify-center text-slate-300 text-[10px] font-bold">
                                NO IMG
                              </div>
                            )}
                          </div>
                          <span className="font-black text-slate-800 text-lg tracking-tight leading-tight">
                            {item.name}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <div
                          className={`inline-block px-4 py-1.5 rounded-xl font-black text-lg ${totalQty > 0 ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-500"}`}
                        >
                          {totalQty}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex justify-center gap-2">
                          {(role === "admin" ||
                            role === "superadmin" ||
                            permissions?.["Product Registration_Edit"] ===
                              true) && (
                            <button
                              onClick={() => editProduct(item)}
                              className="p-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 hover:text-blue-600 hover:border-blue-100 hover:shadow-md transition-all active:scale-90"
                            >
                              <Pencil size={18} />
                            </button>
                          )}
                          ...
                          {(role === "admin" ||
                            role === "superadmin" ||
                            permissions?.["Product Registration_Delete"] ===
                              true) && (
                            <button
                              onClick={() => deleteProduct(item.id)}
                              className="p-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 hover:text-red-500 hover:border-red-100 hover:shadow-md transition-all active:scale-90"
                            >
                              <Trash2 size={18} />
                            </button>
                          )}
                          ...
                          <button
                            ...
                            className="p-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 hover:text-sky-500 hover:border-sky-100 hover:shadow-md transition-all active:scale-90"
                          >
                            <Eye size={18} />
                          </button>
                        </div>
                      ...
                      <td className="px-6 py-4 text-center">
                        {item.batches?.length > 0 ? (
                          ...
                          <span className="text-slate-300 font-bold text-[10px] uppercase tracking-widest italic">
                            No Data
                          </span>
                        )}
                      </td>
      ...
      {showQRModal && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-[#FFF7EF] p-6 rounded-xl shadow-xl text-center w-[400px]">
            ...
            <div
              className="flex justify-center bg-white p-4 rounded-lg mb-4"
              id="qr-code-ref"
            >
              <QRCode
                value={selectedLink}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-md transition-opacity p-4">
          <div className="w-full max-w-[850px] bg-[#FFF7EF] rounded-3xl shadow-2xl overflow-hidden border border-white/20 max-h-[95vh] flex flex-col">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">
                      Manage inventory details and batch assignments.
                    </p>
                  </div>
                </div>
                <button
            ...
            <form
              ...
              className="px-10 pb-8 overflow-y-auto flex-grow custom-scrollbar"
            >
              <div className="space-y-8">
                {/* SECTION: BASIC INFO */}
                <div className="space-y-5">
                  <h3 className="text-[11px] font-bold uppercase tracking-wider text-[#FA9C42] border-b border-[#FA9C42]/10 pb-2">
                    Primary Details
                  </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <label
                          className={`flex-grow flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed cursor-pointer transition-all ${modalMode === "view" ? "hidden" : "bg-white border-slate-200 hover:border-[#FA9C42]"}`}
                        >
                    ...
                    <div>
                      ...
                      <div className="flex gap-4">
                        {["KKW", "TCS"].map((g) => (
                          <label
                            key={g}
                            className={`flex items-center gap-3 px-6 py-3 rounded-xl border-2 transition-all cursor-pointer ${product.godown.includes(g) ? "bg-[#FA9C42] border-[#FA9C42] text-white shadow-md" : "bg-white border-slate-100 text-slate-500 hover:border-slate-200"}`}
                          >
                            <input
                              type="checkbox"
                              className="hidden"
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between border-b border-[#FA9C42]/10 pb-2">
                    <h3 className="text-[11px] font-bold uppercase tracking-wider text-[#FA9C42]">
                      Stock Batches
                    </h3>
                    {modalMode !== "view" && (
                      <button
                  ...
                  <div className="bg-white rounded-2xl border-2 border-slate-100 overflow-hidden">
                    <table className="w-full text-left">
                      <thead className="bg-slate-50">
                        <tr>
                          <th className="px-4 py-3 text-[10px] font-bold text-slate-400 uppercase">
                            Batch No
                          </th>
                          <th className="px-4 py-3 text-[10px] font-bold text-slate-400 uppercase">
                            Quantity
                          ...
                          {modalMode !== "view" && (
                            <th className="px-4 py-3 text-center text-[10px] font-bold text-slate-400 uppercase">
                              Action
                            </th>
                          )}
                        </tr>
      ...
      {showBarcodeModal && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-[#FFF7EF] p-6 rounded-xl shadow-xl text-center w-[400px]">
            ...
            <div
              className="flex justify-center bg-white p-6 rounded-lg mb-4"
              id="barcode-ref"
            >
              <Barcode
                value={selectedBarcode || "Product"}
                height={60}
                width={2}
                fontSize={14}
              />
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">
                    Detailed stock breakdown for this product
                  </p>
                </div>
              </div>
              <button
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {selectedBatches.length > 0 ? (
                      selectedBatches.map((b, index) => (
                        <tr
                          key={index}
                          className="group hover:bg-[#FA9C42]/5 transition-colors"
                        >
                          <td className="px-6 py-4">
                        ...
                        <td colSpan="3" className="px-6 py-12 text-center">
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <p className="text-slate-400 text-sm italic font-medium">
                              No batch data available for this item
                            </p>
                          </div>
                        </td>
                      </tr>
      ...
      {showPrintModal && printProduct && (
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[100] p-4 print:p-0 print:bg-white">
          ...
          <div
            className="bg-[#FFF7EF] w-full max-w-[850px] rounded-3xl shadow-2xl flex flex-col max-h-[700px] print:max-h-none print:shadow-none print:bg-white print:w-full"
            ref={batchRef}
          >
            ...
            <div className="px-8 pb-4 overflow-y-auto flex-grow custom-scrollbar print:overflow-visible print:p-0">
              ...
              <div
                id="print-area"
                className="bg-white p-8 rounded-2xl border border-slate-100 print:border-none print:p-0 text-slate-900"
              >
                ...
                <div className="flex justify-between items-start border-b-2 border-slate-900 pb-6 mb-8">
                  <div>
                    ...
                    <div className="mt-2 text-sm text-slate-500 space-y-0.5 font-medium">
                      <p>
                        Shop no. 18, Business Bay, Mumbai Naka, Nashik - 422002
                      </p>
                      <p>Email: support@theceramicstudio.in</p>
                      <p>Phone: 8847788888 / 7085899999</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-12 mb-10">
                  {[
                    ...
                    <div
                      key={i}
                      className="flex justify-between items-end border-b border-slate-100 pb-1"
                    >
                ...
                {printProduct.batches?.length > 0 ? (
                    ...
                    <table className="w-full border-collapse">
                      <thead>
                        <tr className="bg-slate-50">
                          <th className="border-y border-slate-200 p-3 text-left text-[10px] font-bold uppercase text-slate-500">
                            Batch No.
                          </th>
                          <th className="border-y border-slate-200 p-3 text-center text-[10px] font-bold uppercase text-slate-500">
                            Quantity
                          </th>
                          <th className="border-y border-slate-200 p-3 text-left text-[10px] font-bold uppercase text-slate-500">
                            Storage Location
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {printProduct.batches.map((b, i) => (
                          <tr key={i} className="font-mono text-sm">
                            ...
                            <td className="p-3 text-slate-600 uppercase">
                              {b.location}
                            </td>
                          </tr>
                        ))}
                  ...
                  <div className="py-6 border-2 border-dashed border-slate-100 text-center mb-10">
                    <p className="text-slate-400 text-sm italic">
                      No batch assignments found for this product.
                    </p>
                  </div>
                )}
                ...
                <div className="mt-20 flex justify-between items-end">
                  <div className="text-[10px] text-slate-400 max-w-[250px]">
                    ...
                    <p>
                      This is a computer-generated stock sheet. Verify stock
                      physically before dispatch.
                    </p>
                  </div>
