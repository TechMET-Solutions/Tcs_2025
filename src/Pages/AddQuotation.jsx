...
`); // Your default HTML string here
  ...
  const discountAmount =
    (Number(itemTotal) * (Number(additionalDiscount) || 0)) / 100;
  ...
  const saveQuotation = async () => {
    ...
    if (!clientName.trim() && searchTerm.trim() === "") {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        await axios.post(`${BASEURL}/api/Quotation/saveQuotation`, payload);
      ...
      <div className=" mx-auto">
        ...
        <div className="flex justify-between items-center mb-10 bg-white p-8 rounded-[35px] shadow-sm border border-orange-50">
          ...
          <button
            ...
            className="bg-[#FF7A00] hover:bg-[#E66E00] text-white px-12 py-4 rounded-2xl font-black shadow-xl shadow-orange-100 transition-all flex items-center gap-3 active:scale-95"
          >
            <Save size={20} />{" "}
            {isEditMode ? "UPDATE CHANGES" : "SAVE QUOTATION"}
          </button>
        </div>
          ...
          <div className=" space-y-10">
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
              <div className="flex items-center gap-3 mb-8">
                <div className="p-3 bg-orange-50 rounded-2xl text-[#FF7A00]">
                  <User size={24} />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  {isOpen && suggestions.length > 0 && (
                    <div className=" z-10 max-w-full bg-white border border-slate-200 rounded-md shadow-lg mt-1 max-h-60 overflow-auto">
                        ...
                        <div
                          ...
                          onClick={() => {
                            ...
                            setSelectedArchitect(
                              String(customer.assignedArchitectId || ""),
                            );
                            setAttendedBy(
                              String(customer.assignedEmployeeId || ""),
                            );
                            setSearchTerm(
                              `${customer.name || ""} ${customer.Last_Name || ""}`.trim(),
                            );
                ...
                <div className="space-y-1">
                  ...
                  <input
                    className="lux-input border-orange-100"
                    placeholder="27XXXXX..."
                    value={clientGst}
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                <FileText size={16} className="text-orange-500" /> Introduction
                Note
              </h3>
              <JoditEditor
                value={headerSection}
                config={config}
                onBlur={(c) => setHeaderSection(c)}
              />
            </div>
            ...
            <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden">
              <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
                ...
                <button
                  ...
                  className="bg-white text-orange-600 border border-orange-100 px-6 py-2 rounded-xl font-black text-[10px] tracking-widest uppercase hover:bg-orange-600 hover:text-white transition-all shadow-sm"
                >
                  + Add New Row
                </button>
              </div>
                ...
                <table className="w-full text-left border-collapse table-fixed">
                  ...
                  <thead className="bg-slate-50/50 border-b border-slate-100">
                    <tr>
                      ...
                      <th className="p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-[280px]">
                        Product Search
                      </th>
                      <th className="p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-[120px]">
                        Details
                      </th>
                      ...
                      <th className="p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-[80px] text-center">
                        Weight
                      </th>
                      <th className="p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-[80px] text-center">
                        TWgt
                      </th>
                      <th className="p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-[90px] text-center">
                        Coverage
                      </th>
                      <th className="p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-[80px] text-center">
                        Dis%
                      </th>
                      <th className="p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-[120px] text-right">
                        Total
                      </th>
                      <th className="p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-[80px] text-center">
                        Area
                      </th>
                      <th className="p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-[60px] text-center">
                        Action
                      </th>
                    </tr>
                  </thead>
                    ...
                    {rows.map((r, i) => (
                      ...
                      >
                        ...
                        <td className="p-3 text-center">
                          <input
                            className="table-input w-20 h-10 text-center"
                            value={r.rate}
                            onChange={(e) =>
                        ...
                        <td className="p-3">
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <input
                              className="table-input w-12 h-10 text-center"
                              value={r.box}
                              onChange={(e) =>
                                updateRowField(i, "box", e.target.value)
                              }
                            />
                          </div>
                        </td>
                        <td className="p-3 text-center">
                          <input
                            className="table-input w-16 h-10 text-center text-orange-600"
                            value={r.Weight}
                            onChange={(e) =>
                              updateRowField(i, "Weight", e.target.value)
                            }
                          />
                        </td>
                        <td className="p-3 text-center">
                          <input
                            className="table-input w-16 h-10 text-center text-orange-600 bg-slate-50"
                            value={r.TWgt}
                            readOnly
                          />
                        </td>
                        ...
                        <td className="p-3 text-center">
                          <input
                            ...
                            value={r.discount}
                            onChange={(e) =>
                              updateRowField(i, "discount", e.target.value)
                            }
                          />
                        </td>
                        ...
                        <td className="p-3 text-center">
                          <input
                            ...
                            value={r.area}
                            onChange={(e) =>
                              updateRowField(i, "area", e.target.value)
                            }
                          />
                        </td>
                        <td className="p-3 text-center">
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                <Info size={16} className="text-orange-500" /> Bank Details &
                Terms
              </h3>
              <JoditEditor
                value={bottomSection}
                config={config}
                onBlur={(c) => setBottomSection(c)}
              />
            </div>
                ...
                <div className="bg-gradient-to-br from-[#FFF7F0] to-[#FFFFFF] p-10 rounded-[50px] border border-orange-100 shadow-xl shadow-orange-50/50">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="pt-8 mt-4 border-t-2 border-dashed border-orange-100">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] block mb-2">
                  ...
                  <div
                    className="mt-10 p-5 bg-orange-600 rounded-[25px] text-white text-center font-black text-sm cursor-pointer hover:bg-orange-700 transition-all shadow-lg shadow-orange-200"
                    onClick={() => setShowFeedback(true)}
                  >
          ...
          <div className="fixed inset-0 bg-slate-200/50 backdrop-blur-xl z-[999] flex items-center justify-center p-6">
            <div className="bg-white w-full max-w-2xl rounded-[60px] shadow-2xl p-12 border border-orange-50 relative">
              <button
                ...
                className="absolute top-10 right-10 p-3 bg-slate-50 rounded-full text-slate-400 hover:text-red-500 transition-all"
              >
                <X size={24} />
              </button>
              <div className="text-center mb-10">
                <h2 className="text-3xl font-black text-slate-800 tracking-tight">
                  {isEditMode ? "Updating Quotation" : "Finalizing Quotation"}
                </h2>
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-6 bg-orange-50/50 rounded-[30px] border border-orange-50">
                    <p className="text-[10px] font-black text-orange-400 uppercase mb-1">
                      Total Items Value
                    </p>
                ...
                <button
                  ...
                  className="w-full bg-orange-600 text-white py-5 rounded-[25px] font-black shadow-xl shadow-orange-100 transition-all uppercase text-[10px] tracking-[0.2em]"
                >
