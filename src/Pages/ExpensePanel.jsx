...
const fetchEmpSallaryTransactions = async (empId, page = 1) => {
  if (!empId) return;

  setEmpLoading(true);
  try {
    const res = await fetch(
      `${BASEURL}/api/transactions/salary/history/${empId}?page=${page}`
    );
  ...
  const fetchTransactions = async () => {
    try {
      const response = await fetch(
        `${API_URL}/GetAllTransaction?page=${currentPage}&limit=10&search=${search}`,
      );
...
const handleEmpSearch = (val) => {
  setEmpSearch(val);
  fetchEmployees(val, setEmpList, "salary"); // or any type you want
};
  ...
  const validateForm = () => {
    ...
    if (formData.type === "debit" && enteredAmount > currentStockBalance) {
      alert(
        `Insufficient Balance! Stock is ₹${currentStockBalance.toLocaleString()}`,
      );
      return false;
    }
    return true;
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-2xl font-bold text-[#1F3A93] mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-blue-600">
            <p className="text-sm text-gray-500 uppercase font-bold text-[10px]">
              Main Balance
            </p>
            <p className="text-3xl font-black text-blue-900">
              ₹{Number(summary.current_balance).toLocaleString()}
            </p>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-green-500">
            ...
            <p className="text-2xl font-bold text-green-600">
              ₹{Number(summary.total_added).toLocaleString()}
            </p>
          </div>
              ...
              {historyEmployees.length > 0 && (
                <div className="absolute z-50 w-full bg-white border rounded-md shadow-2xl mt-1 overflow-hidden">
                  {historyEmployees.map((emp) => (
                    <div
                      key={emp.id}
                      onClick={() => selectEmployeeForHistory(emp)}
              ...
              <div className="flex items-center gap-4 bg-blue-50 px-4 py-2 rounded-lg border border-blue-200">
                <div>
                  ...
                  <p className="text-lg font-black text-blue-900">
                    ₹{Number(empBalance).toLocaleString()}
                  </p>
                </div>
                <button
                  onClick={() => {
                  ...
                  {isHistoryLoading ? (
                    ...
                    empTransactions.map((et) => (
                      <tr key={et.id} className="hover:bg-gray-50 transition">
                        ...
                        <td
                          className={`px-6 py-4 text-right font-bold ${et.type === "DEBIT" ? "text-red-600" : "text-green-600"}`}
                        >
                          {et.type === "DEBIT" ? "-" : "+"}₹
                          {Number(et.amount).toLocaleString()}
                        </td>
                      </tr>
                    ))
                  ) : (
            ...
            <div className="relative max-w-2xl w-full bg-white rounded-2xl overflow-hidden shadow-2xl">
              ...
              <div className="p-4 border-b flex justify-between items-center bg-gray-50">
                <span className="font-bold text-gray-700 flex items-center gap-2">
                  <ImageIcon size={18} className="text-blue-600" /> Payment Bill
                  Preview
                </span>
                <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-md h-fit relative">
            ...
            <form onSubmit={handleAddEntry} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full bg-[#1F3A93] text-white py-2.5 rounded-md font-semibold hover:bg-blue-800 transition"
              >
                Save Transaction
              </button>
            </form>
                ...
                <tbody className="divide-y divide-gray-100">
                  {transactions.map((t) => (
                    <tr key={t.id} className="hover:bg-gray-50 transition">
                      ...
                      <td
                        className={`px-6 py-4 text-right font-bold ${t.type === "credit" ? "text-green-600" : "text-red-600"}`}
                      >
                        {t.type === "credit" ? "+" : "-"}₹
  ...
  <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
    <Wallet size={20} /> Employee Transaction Records
  </h2>
  ...
  <div className="relative w-full md:w-1/3">
    ...
    <input
      ...
      onBlur={() => setTimeout(() => setEmpList([]), 200)}
    />
        ...
        {empList.map((emp) => (
          <div
            ...
            onClick={() => {
              ...
              setEmpList([]);
              fetchEmpSallaryTransactions(emp.id); // 🔥 Pass ID to new API
            }}
    ...
    <div className="mt-6 border rounded-lg overflow-hidden">
      <table className="w-full text-left">
        ...
        <tbody className="divide-y">
          {empLoading ? (
            <tr>
              <td colSpan="3" className="py-10 text-center">Loading...</td>
            </tr>
          ) : empRecords.length > 0 ? (
            empRecords.map((r) => (
              <tr key={r.id}>
                ...
                <td className="px-4 py-3 text-sm">{r.note}</td>
