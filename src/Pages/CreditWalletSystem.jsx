...
const CreditLedger = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(`${BASEURL}/api/wallet/stock/${currentUser.id}`);
  ...
  const handleSpendCredit = async () => {
    if (!inputAmount || !billFile) return alert("Please enter amount and upload bill image");
    const formData = new FormData();
    formData.append('employeeId', currentUser.id); // Default 1 jayega
  ...
  if (loading && myData.logs.length === 0) return (
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8 font-sans">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        <div>
          <h1 className="text-xl font-black text-slate-800 tracking-tight">Welcome, {currentUser.name}</h1>
          <p className="text-[10px] text-indigo-500 font-bold uppercase">Staff ID: #00{currentUser.id}</p>
        </div>
        <div className="bg-slate-100 p-2 rounded-xl text-slate-400"><History size={20}/></div>
      </div>

      {/* Real-time Balance Card */}
      <div className="bg-indigo-600 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden">
        <div className="relative z-10">
          <p className="opacity-70 uppercase text-[10px] font-black tracking-[0.2em]">Available Stock Balance</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Spent For</label>
              <input 
                type="text" placeholder="e.g. Travel to Site" 
            ...
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Bill/Receipt</label>
              <label className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer hover:bg-slate-50 transition-colors">
                ...
                <span className="text-[11px] text-slate-500 font-bold">{billFile ? billFile.name : "Tap to Upload Photo"}</span>
                <input type="file" className="hidden" accept="image/*" onChange={(e) => setBillFile(e.target.files[0])} />
              </label>
            </div>
            <button 
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col">
          ...
          <div className="divide-y overflow-y-auto max-h-[480px]">
            {myData.logs.map(log => (
              <div key={log.id} className="p-5 flex justify-between items-center hover:bg-slate-50">
                <div className="flex gap-4 items-center">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold ${log.type === 'CREDIT' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                    {log.type === 'CREDIT' ? '+' : '-'}
                  </div>
                  <div>
                    <p className="font-bold text-slate-800 text-sm">{log.note || 'No Description'}</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase">{new Date(log.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className="text-right flex flex-col items-end gap-2">
                  <span className={`font-mono font-black text-sm ${log.type === 'CREDIT' ? 'text-emerald-500' : 'text-rose-600'}`}>
                    ₹{log.amount}
                  </span>
                  {log.bill_attachment && (
                    <a 
            ...
            {myData.logs.length === 0 && (
              <div className="p-20 text-center text-slate-300 font-bold italic text-xs uppercase tracking-widest">No transactions found</div>
            )}
          </div>
        </div>
      </div>
