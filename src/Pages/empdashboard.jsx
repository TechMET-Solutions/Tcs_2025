...
const EmpDashboard = () => {
  ...
  const fetchEmployeeDashboard = async () => {
    try {
      const res = await axios.get(
        `${BASEURL}/api/users/employee-dashboard/${user?.id}`,
      );

      if (res.data.success) {
        setStats({
          quotationCount: res.data.counts.quotations,
          customerCount: res.data.counts.customers,
        });
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (user?.id) {
      fetchInitialData(); // your existing status/attendance
      fetchMyTasks(); // tasks
      fetchEmployeeDashboard(); // 🔥 new dashboard data
    }
  }, [user]);
  ...
  const PunchControl = () => {
    ...
    return (
      <div className="flex bg-white p-2 rounded-2xl shadow-sm border border-slate-200 gap-3 w-[300px]">
        {/* PUNCH IN BUTTON */}
        <button
          disabled={loading || currentStatus !== "READY"}
          onClick={() => handlePunch("IN")}
          className={`flex-1 px-2 py-2 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${
            currentStatus !== "READY"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-10">
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-amber-500 text-xs font-bold bg-amber-50 px-2 py-1 rounded-lg">
                Action Required
              </span>
            </div>
            <p className="text-slate-500 font-bold text-xs uppercase tracking-widest">
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
            ...
            <h3
              className={`text-2xl font-black mt-2 ${currentStatus === "IN" ? "text-green-600" : currentStatus === "COMPLETED" ? "text-slate-400" : "text-blue-600"}`}
            >
              {currentStatus === "IN"
                ? "ON DUTY"
          ...
          <section className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-white">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="px-3 py-1 bg-amber-100 text-amber-600 text-[10px] font-black rounded-full uppercase">
                  Pending
                </span>
              </div>
            </div>
                    ...
                    tasks.map((task) => (
                      ...
                      >
                        <td className="px-8 py-5">
                          ...
                          <div className="text-xs text-slate-400 mt-1 max-w-md italic">
                            "{task.description}"
                          </div>
                          {task.remark &&
                            (() => {
                              ...
                              return arr.length ? (
                                <div className="mt-2 space-y-1">
                                  {arr.map((r, i) => (
                                    <div
                                      key={i}
                                      className="text-[10px] bg-indigo-50 text-indigo-500 p-2 rounded-lg font-medium"
                                    >
