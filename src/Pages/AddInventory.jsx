...
export default function AddInventory() {
  ...
  useEffect(() => {
    if (isEditMode && editData && products.length > 0) {
      ...
      setPurchaseMeta({
        purchaseDate:
          editData.purchase_date?.slice(0, 10) ||
          new Date().toISOString().slice(0, 10),
        clientName: editData.client_name || "",
        clientContact: editData.client_contact || "",
        purchaseId: editData.id,
        billNo: editData.bill_no || "",
      });
      ...
      if (editData.items && editData.items.length > 0) {
        const mappedRows = editData.items.map((item) => {
          ...
          return {
            ...
            cov: item.cov || 1,
            batchNo: item.batch_no || "",
            availQty: item.inStock || 0, // DB ka stock (jaise 550)
  ...
  const applyProductToRow = (rows, i, product) => {
    rows[i] = {
      ...rows[i],
      ...
      total: 0,
      batchNo: "",
      filteredProducts: rows[i].filteredProducts || [],
    };
  };
    ...
    const fetchSuppliers = async () => {
      ...
      try {
        const response = await fetch(
          `${BASEURL}/api/suppliers/list?search=${searchTerm}`,
        );
  ...
  const selectProductFromSearch = (i, product) => {
    ...
    updated[i] = {
      ...updated[i],
      productName: product.name,
      filteredProducts: sameNameProducts, // only same-name products
      productId: "",
      size: "",
      quality: "",
      rate: "",
  ...
  const handleBatchInput = (i, value) => {
    ...
    const existing = updated[i].batches.find(
      (b) => b.batch_no.toLowerCase() === value.toLowerCase(),
    );
  ...
  const savePurchase = async (e) => {
    ...
    //   subTotal,
    // };
    const hasProduct = rows.some((r) => r.productId && r.productId !== "");
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
          <div>
            <h1
              ...
              style={{ color: SIDEBAR_DARK }}
            ></h1>
            <p className="text-slate-500 font-medium mt-1">
              Record new arrivals into the inventory ledger
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              type="submit"
              className="text-white px-10 py-4 rounded-[20px] font-black transition-all shadow-xl flex items-center gap-2 hover:scale-105 active:scale-95"
        ...
        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 mb-8 grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="space-y-2">
            ...
            <input
              name="purchaseDate"
              type="date"
              value={purchaseMeta.purchaseDate}
          ...
          <div className="space-y-2">
            ...
            <input
              name="billNo"
              placeholder="Ex: BILL-101"
              value={purchaseMeta.billNo}
              onChange={handleMetaChange}
              className="metaInput"
            />
          </div>
          <div className="space-y-2 relative">
            ...
            <input
              ...
              value={searchTerm || purchaseMeta.clientName} // Controlled by searchTerm state
              onChange={(e) => {
          ...
          <div className="space-y-2">
            ...
            <input
              name="clientContact"
              placeholder="Mobile / Ref"
              value={purchaseMeta.clientContact}
              onChange={handleMetaChange}
              className="metaInput"
            />
          </div>
        ...
        <div className="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  {[
                    ...
                    <th
                      key={h}
                      className="p-5 text-[10px] font-black uppercase text-slate-400 tracking-widest whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-slate-50 p-5">
                {rows.map((r, i) => (
                  <tr
                    key={i}
                    className="group hover:bg-slate-50/30 transition-colors h-[100px]"
                  >
                    ...
                    <td className="p-3">
                      <input
                        ...
                        className="tableInput w-24 font-black text-orange-600"
                      />
                    </td>
                    <td className="p-3">
                      <input
                    ...
                    <td className="p-3">
                      <input
                        value={r.qty}
                        onChange={(e) =>
                          updateRowField(i, "qty", e.target.value)
                        }
                        className="tableInput w-20 font-black bg-yellow-50 border-yellow-200 focus:border-yellow-500"
                      />
                    </td>

                    <td className="p-3 font-black text-slate-800 text-sm whitespace-nowrap">
                    ...
                    <td className="p-3">
                      <select
                        value={r.godown}
                        onChange={(e) =>
                          updateRowField(i, "godown", e.target.value)
                        }
                        className="tableInput w-24 appearance-none font-bold bg-white"
                      >
                        <option>KKW</option>
                        <option>TCS</option>
                      </select>
          ...
          <div className="p-6 bg-slate-50/50 border-t border-slate-100 flex justify-between items-center">
            <button
              type="button"
              onClick={addRow}
              className="flex items-center gap-2 text-[#FF7A00] font-black text-xs uppercase tracking-widest bg-white px-6 py-3 rounded-2xl border border-slate-200 hover:border-orange-200 transition-all shadow-sm"
            >
              <Plus size={18} /> Add Product Row
            </button>
            <div className="flex items-center gap-2 text-slate-400 text-[10px] font-black uppercase tracking-widest">
              <Info size={14} style={{ color: BRAND_ORANGE }} /> Calculations
        ...
        {dropdownPos.rowIndex !== null && (
          ...
          >
              ...
              (rows[dropdownPos.rowIndex].filteredProducts.length > 0 ? (
                rows[dropdownPos.rowIndex].filteredProducts.map((p) => (
                  <div
                    key={p.id}
                    onClick={() =>
                      selectProductFromSearch(dropdownPos.rowIndex, p)
                    }
            ...
            {dropdownPos.type === "batch" && (
              <>
                <div className="p-2 bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b">
                ...
                {rows[dropdownPos.rowIndex].batches
                  .filter((b) =>
                    b.batch_no
                      .toLowerCase()
                      .includes(
                        rows[dropdownPos.rowIndex].batchNo.toLowerCase(),
                      ),
                  )
                  .map((b, idx) => (
                    <div
                      key={idx}
                      onClick={() => selectBatch(dropdownPos.rowIndex, b)}
                      className="p-3 hover:bg-orange-50 cursor-pointer flex justify-between items-center border-b border-slate-50 transition-colors"
                    >
                      <span className="text-sm font-bold text-slate-700">
                        {b.batch_no}
                      </span>
                      <span className="text-[10px] font-black bg-orange-100 text-orange-600 px-2 py-0.5 rounded">
                        Stock: {b.qty}
                      </span>
                    </div>
                  ))}
