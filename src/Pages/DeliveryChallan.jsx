...
import { Activity, ClipboardList, MapPin, Printer, Trash2, Truck, Search, X } from "lucide-react";
import { useEffect, useState, useCallback } from "react";
import { getAllDeliveryChallan } from "../Component/API/quotationApi";
import { addTracking, deleteTracking, getTrackingByChallan } from "../Component/API/trackingApi";
...
export default function DeliveryChallan() {
  ...
  const printChallan = (id) => window.open(`${BASEURL}/api/Quotation/delivery-challan/print/${id}`, "_blank");
  const printChallan2 = (id) => window.open(`${BASEURL}/api/Quotation/delivery-challan/printreturn/${id}`, "_blank");
    ...
    <div className="min-h-screen bg-[#FDFDFF] p-6 md:p-12 font-['Lexend'] text-slate-800">
      ...
      <div className="max-w-[1500px] mx-auto mb-10 flex flex-col lg:flex-row justify-between items-center gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl font-black text-slate-900">Delivery <span className="text-orange-500 underline underline-offset-8">Challans</span></h1>
        </div>

        {/* SEARCH BAR */}
        <form onSubmit={handleSearchTrigger} className="flex w-full max-w-lg gap-2">
          ...
          <button type="submit" className="px-6 py-3 bg-slate-900 text-white rounded-2xl font-bold hover:bg-orange-500 transition-all active:scale-95">
            Search
          </button>
        </form>
        ...
        <div className="bg-gray-100 px-4 py-2 rounded-2xl flex items-center gap-4 border border-slate-200">
          ...
          <div>
            <span className="block text-lg font-black text-slate-900">{challanList.length}</span>
            <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Showing results</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1500px] mx-auto bg-white shadow-xl shadow-slate-200 rounded-[24px] overflow-hidden border border-slate-100">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr><td colSpan="6" className="text-center py-20 font-bold text-slate-400 animate-pulse">Loading Data...</td></tr>
              ) : challanList.length === 0 ? (
                <tr>
                  <td colSpan="6" className="text-center py-40">
                    <div className="flex flex-col items-center gap-3 opacity-20">
                      <Truck size={64} />
                      <span className="text-2xl font-black italic">No Records Found</span>
                    </div>
                  </td>
                </tr>
              ) : (
                challanList.map((item) => (
                  <tr key={item.id} className="group hover:bg-slate-50/60 transition-colors">
                    <td className="px-8 py-5 font-black text-slate-900">CH-{item.id}</td>
                    <td className="px-8 py-5 font-bold text-slate-600">{item.quotationId}</td>
                    <td className="px-8 py-5 font-bold text-slate-800">{item.client}</td>
                    <td className="px-6 py-5 text-center">
                      <button onClick={() => { setTrackingChallanId(item.id); setShowTrackingModal(true); }} className="px-3 py-1.5 rounded-full border border-slate-200 text-[10px] font-black uppercase hover:bg-slate-900 hover:text-white transition-all">
                        Update Timeline
                      </button>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <button onClick={() => handleDeleteItem(item.id)} className="p-2.5 rounded-xl bg-red-50 text-red-400 hover:bg-red-500 hover:text-white transition-all">
                        <Trash2 size={16} />
                      </button>
                    </td>
                    <td className="px-8 py-5 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => printChallan(item.id)} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-full text-[10px] font-black uppercase hover:bg-orange-500 transition-all">
                          <Printer size={14} /> DC
                        </button>
                        <button onClick={() => printChallan2(item.id)} className="p-2 text-slate-400 hover:text-slate-900 transition-colors">
                          <Printer size={16} />
                        </button>
                      </div>
                    </td>
        ...
        <div className="px-8 py-6 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="text-xs font-bold text-slate-400 uppercase">Page {currentPage} of {totalPages}</div>
      ...
      {/* ... (Keep your Tracking Modal code as is) ... */}
    </div>
  );
}