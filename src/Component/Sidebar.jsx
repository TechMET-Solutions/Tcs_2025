...
export default function Sidebar() {
  ...
  const NavItem = ({ label, icon, path, isSubItem = false }) => (
    ...
    >
      <span
        className={`${isSubItem ? "text-base" : "text-lg"} ${!open ? "scale-110" : ""}`}
      >
        {icon}
      </span>
  ...
  return (
    <div
      className={`h-screen ${open ? "w-72 rounded-tr-[50px] rounded-br-[50px]" : "w-20"} 
    ...
    >
      ...
      <div className="flex items-center justify-between p-5 min-h-[80px]">
        {open ? (
          <img
            src={LOGO}
            alt="logo"
            className="w-[75%] transition-all duration-300 opacity-100"
          />
        ) : (
      ...
      <div
        className="
    flex-1 overflow-y-auto px-3 pt-4
    [&::-webkit-scrollbar]:w-2
    [&::-webkit-scrollbar-track]:bg-transparent
    [&::-webkit-scrollbar-thumb]:bg-[#555]
    [&::-webkit-scrollbar-thumb]:rounded-full
    hover:[&::-webkit-scrollbar-thumb]:bg-[#777]
  "
      >
        ...
        {hasView("Architect Registration") && (
          <NavItem
            label="Architect Registration"
            icon={<DraftingCompass />}
            path="/Architect-Registration"
          />
        )}
